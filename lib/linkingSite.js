...
function attachLinkingSiteRoutes(app, deps = {}) {
  ...
  const aliases = Array.from(new Set([routeBase, '/Freebot']));
...
<body>
  ...
  <aside id="sidebar" class="sidebar"><div class="sidebar-inner">
    ...
    <div class="side-card">
      <h4>معلومات البوت</h4>
      <div class="side-list">
        <div class="side-item"><span>الحالة</span><strong id="sideStatus">Loading...</strong></div>
        <div class="side-item"><span>الوقت</span><strong id="sideTime">--:--:--</strong></div>
        <div class="side-item"><span>الأرقام المربوطة</span><strong id="sideLinked">0</strong></div>
        <div class="side-item"><span>المستخدمون</span><strong id="sideUsers">0</strong></div>
      </div>
    </div>
    <div class="side-card">
      <h4>الروابط السريعة</h4>
      <div class="side-btns">
        <button class="side-btn" id="goHomeBtn">🏠 الصفحة الرئيسية</button>
        <button class="side-btn" id="openSettingsBtn">⚙️ الإعدادات</button>
        <button class="side-btn" id="openCommandsSideBtn">📦 أقسام الأوامر</button>
  ...
  <main class="page">
    ...
    <section class="hero">
      <div class="hero-grid">
        <div>
          <h1>${siteName}</h1>
          <p>واجهة مستقلة مضافة داخل مشروع <strong>KnightBot MD</strong> بنفس فكرة صفحة <strong>Freebot</strong>، ومربوطة مباشرة مع البوت وواجهة الإعدادات وواجهات Pair/QR الحالية داخل المشروع.</p>
          <div class="stats-row">
            <div class="stat"><div class="value" id="activeBots">0</div><div class="label">البوتات النشطة</div></div>
    ...
    <section class="components">
      ...
      <p>كل عنصر هنا مربوط فعليًا بما هو موجود في المشروع: الربط، QR، الإعدادات، وحالة البوت الحالية.</p>
...
async function loadSummary(){
  try {
    ...
    $('#activeBots').textContent = data.activeBots ?? 0;
    $('#linkedNumbers').textContent = data.totalLinkedNumbers ?? 0;
    $('#totalUsers').textContent = data.totalUsers ?? 0;
    $('#sideLinked').textContent = data.totalLinkedNumbers ?? 0;
    $('#sideUsers').textContent = data.totalUsers ?? 0;
    $('#sideStatus').textContent = (data.activeBots || 0) > 0 ? 'Online' : 'Idle';
    $('#pairEndpoint').textContent = data.pairing?.endpoint || '/api/pairing';
    $('#settingsEndpoint').textContent = data.settingsUrl || CONFIG.settingsUrl;
    $('#websiteEndpoint').textContent = data.pairing?.website || location.origin;
    $('#routeText').textContent = data.route || CONFIG.basePath;
  } catch (_) {
    $('#sideStatus').textContent = 'Unavailable';
  }
}
...
loadSummary(); loadComments(); tickClock(); setInterval(tickClock, 1000); setInterval(spawnHeart, 650);
</script>
</body>
</html>`;
  }
