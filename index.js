...
const LOG_THROTTLE_SUMMARY_INTERVAL_MS = Math.max(5000, Number(process.env.LOG_THROTTLE_SUMMARY_INTERVAL_MS || 30000));
...
let logThrottleLastSummaryAt = 0;

function serializeLogChunkForThrottle(value, depth = 0) {
    ...
    if (Array.isArray(value)) {
        return `[${value.slice(0, 8).map((item) => serializeLogChunkForThrottle(item, depth + 1)).join(', ')}${value.length > 8 ? ', …' : ''}]`;
    }
    const cloned = {};
    let count = 0;
    for (const [key, item] of Object.entries(value)) {
        if (count >= 12) {
            cloned.__truncated__ = true;
            break;
        }
        cloned[key] = Buffer.isBuffer(item)
            ? `<Buffer ${item.length} bytes>`
            : (item instanceof Error ? (item.stack || item.message || String(item)) : (typeof item === 'object' && item !== null ? serializeLogChunkForThrottle(item, depth + 1) : item));
        count += 1;
    }
    try {
...
function flushSuppressedLogsSummary(force = false) {
    const now = Date.now();
    if (!force && now - logThrottleLastSummaryAt < LOG_THROTTLE_SUMMARY_INTERVAL_MS) return;
    logThrottleLastSummaryAt = now;
    let suppressedTotal = 0;
    for (const [signature, entry] of Array.from(logThrottleState.entries())) {
        if (now - entry.startedAt > LOG_THROTTLE_WINDOW_MS) {
            suppressedTotal += Number(entry.suppressed || 0);
            logThrottleState.delete(signature);
        }
    }
    if (suppressedTotal > 0) {
        ORIGINAL_CONSOLE_METHODS.warn(`[log-throttle] suppressed ${suppressedTotal} repeated log entries in the last window`);
    }
}

function installConsoleRateLimiter() {
    if (console.__rateLimiterInstalled) return;
    for (const method of ['log', 'info', 'warn', 'error']) {
        console[method] = (...args) => {
            try {
                const now = Date.now();
                const signature = `${method}:${args.slice(0, 3).map((item) => serializeLogChunkForThrottle(item)).join(' | ').slice(0, 600)}`;
                const current = logThrottleState.get(signature);
                if (!current || now - current.startedAt > LOG_THROTTLE_WINDOW_MS) {
                    if (current?.suppressed) {
                        ORIGINAL_CONSOLE_METHODS.warn(`[log-throttle] ${current.suppressed} duplicate entries skipped for ${method}`);
                    }
                    logThrottleState.set(signature, { count: 1, suppressed: 0, startedAt: now });
                    ORIGINAL_CONSOLE_METHODS[method](...args.map((item) => serializeLogChunkForThrottle(item)));
                    flushSuppressedLogsSummary(false);
                    return;
                }
                current.count += 1;
                if (current.count <= LOG_THROTTLE_MAX_PER_SIGNATURE) {
                    ORIGINAL_CONSOLE_METHODS[method](...args.map((item) => serializeLogChunkForThrottle(item)));
                } else {
                ...
                ORIGINAL_CONSOLE_METHODS[method](...args);
            }
        };
    }
    console.__rateLimiterInstalled = true;
}
...
const EMBEDDED_PAIR_CODE_BRIDGE = (() => {
    try {
        ...
        const parsedAdminId = parseInt(process.env.ADMIN_ID || "0", 10);
        const ADMIN_ID = Number.isNaN(parsedAdminId) ? 0 : parsedAdminId;

        const GREEN_API_BASE_URL = String(process.env.GREEN_API_BASE_URL || "https://api.green-api.com")
            .trim()
            .replace(/\/+$/, "");
        ...
        function normalizeSettings(data) {
            const output = { ...DEFAULT_SETTINGS, ...(data || {}) };

            output.pair_code_api_method = String(output.pair_code_api_method || "POST").trim().toUpperCase() || "POST";
            if (!["GET", "POST"].includes(output.pair_code_api_method)) {
                output.pair_code_api_method = "POST";
            }

            output.pair_code_api_number_field =
                String(output.pair_code_api_number_field || "phone").trim() || "phone";
            output.pair_code_api_url = String(output.pair_code_api_url || "").trim();
            output.pair_code_api_token = String(output.pair_code_api_token || "").trim();
            output.current_emoji = String(output.current_emoji || "🔥").trim().slice(0, 10) || "🔥";
            output.auto_reply_enabled = parseBool(output.auto_reply_enabled, true);
        ...
        function registerUser(msgOrQuery) {
            const user = msgOrQuery?.from || msgOrQuery?.message?.from;
            if (user?.id) {
                BOT_STATS.totalUsers.add(user.id);
            }
        }

        function isAdmin(msgOrQuery) {
            const user = msgOrQuery?.from || msgOrQuery?.message?.from;
            return Boolean(user && ADMIN_ID && user.id === ADMIN_ID);
        }
        ...
        return {
            ...
            set settings(nextValue) {
                SETTINGS = normalizeSettings(nextValue);
            }
        };
    } catch (error) {
        console.error('Embedded pair-code bridge bootstrap error:', error.message || error);
        return {
            bootstrapError: String(error?.message || error || 'unknown_error')
        };
    }
})();
...
const ALLOW_GLOBAL_SITE_PASSWORD = ['1', 'true', 'yes', 'on'].includes(String(process.env.ALLOW_GLOBAL_SITE_PASSWORD || 'false').trim().toLowerCase());
...
const DEFAULT_SITE_SETTINGS_PAYLOAD = {
    ...
    ownerNumber: String(process.env.DEFAULT_OWNER_NUMBER || DEVELOPER_WHATSAPP_NUMBER || '').trim(),
...
const IMPORTED_OMAR_PHONE = '967771163825';
const IMPORTED_OMAR_PASSWORD = 'UUB010';
const IMPORTED_OMAR_PHONE_SETTINGS = {
    ...DEFAULT_PHONE_SETTINGS,
    name: 'Golden Queen Bot',
    ownername: 'Golden Queen Bot',
    ownerNumber: IMPORTED_OMAR_PHONE,
    from: 'اليمن',
    age: '24',
    prefix: '.',
    footer2: 'Golden Queen Bot',
    description: 'Golden Queen Bot - Yemen',
    mode: 'public',
    alwaysOnline: 'on',
    antiCall: 'off',
    antiDelete: 'inbox',
...
function getImportedPhoneSettingsSeed(phone) {
    ...
    if (normalized === IMPORTED_REDQUEEN_PHONE) {
        return JSON.parse(JSON.stringify(IMPORTED_REDQUEEN_PHONE_SETTINGS));
    }
    if (normalized === IMPORTED_OMAR_PHONE) {
        return JSON.parse(JSON.stringify(IMPORTED_OMAR_PHONE_SETTINGS));
    }
    return null;
}
...
function buildStatusMediaDocId(scope, ownerKey) {
    const seed = [String(scope || 'status-media').trim(), String(ownerKey || '').trim()].join('|');
    return crypto.createHash('sha1').update(seed).digest('hex');
}

async function deleteStatusMediaDocument(mediaId = '') {
    const normalizedMediaId = String(mediaId || '').trim();
    if (!normalizedMediaId) return false;
    const collection = await getMongoCollectionSafely(STATUS_MEDIA_COLLECTION);
    if (!collection) return false;
    try {
        await collection.deleteOne({ _id: normalizedMediaId });
        return true;
    } catch (error) {
        console.error(`⚠️ تعذر حذف وسائط الحالة ${normalizedMediaId} من MongoDB:`, error.message || error);
        return false;
    }
}

async function storeStatusMediaBuffer(scope, ownerKey, entry = {}, buffer, extra = {}) {
    if (!Buffer.isBuffer(buffer) || !buffer.length) return null;
    const collection = await getMongoCollectionSafely(STATUS_MEDIA_COLLECTION);
    if (!collection) return null;

    const normalizedScope = String(scope || 'status-media').trim() || 'status-media';
    const normalizedOwnerKey = String(ownerKey || '').trim() || crypto.randomUUID();
    const mediaId = buildStatusMediaDocId(normalizedScope, normalizedOwnerKey);
    const contentSha1 = crypto.createHash('sha1').update(buffer).digest('hex');
    const fileName = String(entry?.fileName || extra?.fileName || '').trim() || `${mediaId}.bin`;
    const createdAtValue = extra?.createdAt || entry?.createdAt || new Date().toISOString();
    const expiresAtValue = extra?.expiresAt || entry?.deletedExpiresAt || entry?.expiresAt || null;
    ...
    try {
        await collection.updateOne(
            { _id: mediaId },
            {
                $set: {
                    ...
                    archiveId: String(entry?.id || extra?.archiveId || '').trim(),
                    phone: normalizePhone(entry?.phone || extra?.phone || ''),
                    participant: normalizeWhatsAppJid(entry?.participant || extra?.participant || ''),
                    participantPhone: normalizePhone(entry?.participantPhone || extra?.participantPhone || ''),
                    messageId: String(entry?.messageId || extra?.messageId || '').trim(),
                    kind: String(entry?.kind || extra?.kind || '').trim(),
                    mimetype: String(entry?.mimetype || extra?.mimetype || 'application/octet-stream').trim() || 'application/octet-stream',
                    fileName,
        ...
        return {
            ...
            mediaSha1: contentSha1,
            fileName
        };
    } catch (error) {
        console.error(`⚠️ تعذر حفظ وسائط الحالة ${normalizedOwnerKey} في MongoDB:`, error.message || error);
        return null;
    }
}

async function persistStatusMediaFromBuffer(scope, ownerKey, entry = {}, buffer, extra = {}) {
    const stored = await storeStatusMediaBuffer(scope, ownerKey, entry, buffer, extra);
    if (stored?.mediaId) {
        entry.mediaId = stored.mediaId;
        entry.mediaStorage = stored.mediaStorage;
        entry.mediaSize = stored.mediaSize;
        entry.mediaSha1 = stored.mediaSha1;
        entry.fileName = String(entry.fileName || stored.fileName || '').trim();
        entry.filePath = '';
        return true;
    }

    if (!ALLOW_STATUS_MEDIA_FILE_FALLBACK) {
        return false;
    }

    const fallbackFileName = String(entry.fileName || `${sanitizeFileFragment(ownerKey)}.bin`).trim();
    const fallbackPath = path.join(STATUS_MEDIA_DIR, fallbackFileName);
    fs.writeFileSync(fallbackPath, buffer);
    entry.fileName = fallbackFileName;
    entry.filePath = fallbackPath;
    entry.mediaId = '';
    entry.mediaStorage = 'filesystem';
    entry.mediaSize = buffer.length;
    entry.mediaSha1 = crypto.createHash('sha1').update(buffer).digest('hex');
    return true;
}

function normalizeMongoBinaryValue(value) {
...
async function getStatusMediaBuffer(entry = {}) {
    const mediaId = String(entry?.mediaId || '').trim();
    if (mediaId) {
        const collection = await getMongoCollectionSafely(STATUS_MEDIA_COLLECTION);
        if (collection) {
            try {
                const doc = await collection.findOne({ _id: mediaId }, { projection: { data: 1 } });
                const buffer = normalizeMongoBinaryValue(doc?.data);
                if (buffer?.length) return buffer;
            } catch (error) {
                console.error(`⚠️ تعذر تحميل وسائط الحالة ${mediaId} من MongoDB:`, error.message || error);
            }
        }
    }
...
async function migrateStatusMediaEntriesToMongo() {
    const migrateEntries = async (db, collectionName) => {
        let changed = false;
        for (const [entryKey, entry] of Object.entries(db?.items || {})) {
            if (!entry || entry.kind === 'text' || entry.mediaId || !entry.filePath || !fs.existsSync(entry.filePath)) continue;
            try {
                const buffer = fs.readFileSync(entry.filePath);
                if (!buffer.length) continue;
                const persisted = await persistStatusMediaFromBuffer(collectionName, entry.id || entryKey, entry, buffer, {
                    archiveId: entry.id || '',
                    createdAt: entry.createdAt,
                    expiresAt: entry.deletedExpiresAt || entry.expiresAt || null,
                    phone: entry.phone,
                ...
                entry.filePath = '';
                db.items[entryKey] = entry;
                changed = true;
            } catch (error) {
                console.error(`⚠️ تعذر ترحيل وسائط الحالة ${entry.id || entryKey} إلى MongoDB:`, error.message || error);
            }
        }
        return changed;
    };

    const backupsDb = getStatusBackupsDB();
    const archiveDb = getStatusArchiveDB();
    const backupsChanged = await migrateEntries(backupsDb, 'status_backup_media');
    const archiveChanged = await migrateEntries(archiveDb, 'status_archive_media');
    if (backupsChanged) saveStatusBackupsDB(backupsDb);
    if (archiveChanged) saveStatusArchiveDB(archiveDb);
}

const JSON_MIRROR_FILE_KEYS = new Map();
const JSON_MIRROR_CACHE = new Map();
const JSON_MIRROR_DEFAULTS = new Map();
...
function getJsonMirrorPathKey(filePath) {
    return path.resolve(filePath);
}

function registerJsonMirrorDefault(filePath, defaultValue) {
    const pathKey = getJsonMirrorPathKey(filePath);
    if (!JSON_MIRROR_DEFAULTS.has(pathKey)) {
        JSON_MIRROR_DEFAULTS.set(pathKey, cloneJsonValue(defaultValue));
    }
    if (!JSON_MIRROR_CACHE.has(pathKey)) {
        JSON_MIRROR_CACHE.set(pathKey, cloneJsonValue(defaultValue));
    }
    return cloneJsonValue(JSON_MIRROR_CACHE.get(pathKey));
}

function getJsonMirrorDefault(filePath, fallback = {}) {
    const pathKey = getJsonMirrorPathKey(filePath);
    if (JSON_MIRROR_DEFAULTS.has(pathKey)) {
        return cloneJsonValue(JSON_MIRROR_DEFAULTS.get(pathKey));
    }
    return cloneJsonValue(fallback);
}

function getJsonMirrorKey(filePath) {
    const normalized = getJsonMirrorPathKey(filePath);
    if (!JSON_MIRROR_FILE_KEYS.has(normalized)) {
        JSON_MIRROR_FILE_KEYS.set(normalized, path.basename(normalized));
    }
    return JSON_MIRROR_FILE_KEYS.get(normalized);
}

function readJsonFileFromDisk(filePath, fallback = {}) {
    try {
        if (!fs.existsSync(filePath)) return cloneJsonValue(fallback);
        const raw = fs.readFileSync(filePath, 'utf8');
        if (!raw.trim()) return cloneJsonValue(fallback);
...
`, 'utf8');
        ...
        console.error(`⚠️ تعذر كتابة الملف ${path.basename(filePath)} محلياً:`, error.message || error);
        return false;
    }
}

async function persistJsonMirror(filePath, data) {
    const pathKey = getJsonMirrorPathKey(filePath);
...
function getSessionStorageDir(phone = '') {
    const raw = String(phone || '').trim();
    if (!raw) return path.join(SESSIONS_DIR, 'default');
    if (raw === '__web_qr__') return path.join(SESSIONS_DIR, '__web_qr__');
    const normalizedPhone = normalizePhone(raw);
    if (normalizedPhone) return getSessionPath(normalizedPhone);
    const safeName = raw.replace(/[^a-zA-Z0-9._-]+/g, '_') || 'default';
    return path.join(SESSIONS_DIR, safeName);
}

function getLocalSessionMetaFile(phone = '') {
    return path.join(getSessionStorageDir(phone), 'session-meta.json');
}

function readLocalSessionMeta(phone = '') {
    const raw = readJsonFileFromDisk(getLocalSessionMetaFile(phone), {});
    return {
...
async function listLocalSessionEntries() {
    try {
        ...
        const dirs = fs.readdirSync(SESSIONS_DIR, { withFileTypes: true })
            ...
            .filter((name) => name !== '__web_qr__');

        const entries = [];
        for (const dirName of dirs) {
            const phone = normalizePhone(dirName);
            if (!phone) continue;
            const meta = readLocalSessionMeta(phone);
            const sessionDir = getSessionStorageDir(phone);
            const hasAuthFiles = fs.existsSync(path.join(sessionDir, 'creds.json')) || fs.existsSync(path.join(sessionDir, 'app-state-sync-key-1.json')) || fs.existsSync(getLocalSessionMetaFile(phone));
            if (!hasAuthFiles) continue;
            entries.push({
                sessionId: phone,
                phone,
                ownerId: String(meta?.ownerId || getPhoneOwner?.(phone) || '').trim(),
                registered: meta?.registered === true,
                lastConnectedAt: meta?.lastConnectedAt || meta?.updatedAt || null
            });
        }

        entries.sort((a, b) => (Date.parse(b?.lastConnectedAt || 0) || 0) - (Date.parse(a?.lastConnectedAt || 0) || 0));
        return entries;
    } catch (error) {
        console.error('Local Session Scan Error:', error.message || error);
        return [];
    }
}
...
function extractSessionIdFromMongoDocument(doc = {}) {
    const queue = [doc?.sessionId, doc?.phone, doc?.id, doc?._id, doc?.jid, doc?.number];
    for (const candidate of queue) {
        if (candidate == null) continue;
        const normalized = normalizePhone(candidate);
        if (normalized) return normalized;
        if (typeof candidate === 'string') {
            const match = candidate.match(/(\d{6,20})/);
            if (match && match[1]) {
                const extracted = normalizePhone(match[1]);
...
function ensureFile(filePath, defaultValue) {
    registerJsonMirrorDefault(filePath, defaultValue);
    const pathKey = getJsonMirrorPathKey(filePath);
    if (!fs.existsSync(filePath)) {
        writeJsonFileToDisk(filePath, cloneJsonValue(defaultValue));
        JSON_MIRROR_CACHE.set(pathKey, cloneJsonValue(defaultValue));
        return cloneJsonValue(defaultValue);
    }
    const loaded = readJsonFileFromDisk(filePath, defaultValue);
    JSON_MIRROR_CACHE.set(pathKey, cloneJsonValue(loaded));
...
function readJSON(filePath, fallback = {}) {
    ...
    if (fs.existsSync(filePath)) {
        const loaded = readJsonFileFromDisk(filePath, fallback);
        JSON_MIRROR_CACHE.set(pathKey, cloneJsonValue(loaded));
        return cloneJsonValue(loaded);
    }
    return ensureFile(filePath, fallback);
}
...
async function rebuildLinkedUsersFromStoredSessions() {
    const entries = await getStoredSessionEntries();
    for (const entry of entries) {
        if (entry.ownerId) {
            addLinkedNumber(entry.ownerId, entry.phone);
        }
    }
    return entries;
}

async function resetRuntimePhoneDataOnBoot() {
    try {
        if (!PRESERVE_PERSISTENT_RUNTIME_DATA) {
            console.warn('⚠️ PRESERVE_PERSISTENT_RUNTIME_DATA=false -> سيتم مسح البيانات المؤقتة والدائمة عند الإقلاع.');
            invalidateUsersDBCache();
            writeJSON(USERS_FILE, { users: {}, phoneOwners: {} });
            writeJSON(PHONE_SETTINGS_FILE, { profiles: {} });
            writeJSON(STATUS_BACKUPS_FILE, { items: {} });
            writeJSON(STATUS_ARCHIVE_FILE, { items: {} });
            writeJSON(PROFILE_SCHEDULE_FILE, { phones: {} });
            writeJSON(CONTACTS_ARCHIVE_FILE, { phones: {} });
            writeJSON(DELETED_MESSAGES_ARCHIVE_FILE, { items: {} });
            try { fs.rmSync(PHONE_PROFILES_DIR, { recursive: true, force: true }); } catch (_) {}
            try { fs.rmSync(UPLOADS_DIR, { recursive: true, force: true }); } catch (_) {}
            try { fs.rmSync(STATUS_MEDIA_DIR, { recursive: true, force: true }); } catch (_) {}
            ensureDir(PHONE_PROFILES_DIR);
            ensureDir(UPLOADS_DIR);
            ensureDir(STATUS_MEDIA_DIR);
            return;
        ...
        console.error('Runtime Phone Reset Error:', error.message || error);
    }
}
...
function getSettings() {
    ...
    const legacyStartMessages = new Set([
        'مرحباً بك في نظام Golden Queen Bot المتكامل!\nالإيموجي الحالي: {emoji}',
    ...
    settings.pairingApiUrl = String(settings.pairingApiUrl || DEFAULT_PAIRING_API_URL).trim() || DEFAULT_PAIRING_API_URL;
    settings.pairingApiMethod = String(settings.pairingApiMethod || DEFAULT_PAIRING_API_METHOD).trim().toUpperCase() || DEFAULT_PAIRING_API_METHOD;
    if (!PAIRING_API_METHODS.includes(settings.pairingApiMethod)) settings.pairingApiMethod = DEFAULT_PAIRING_API_METHOD;
    settings.pairingApiNumberField = String(settings.pairingApiNumberField || DEFAULT_PAIRING_API_NUMBER_FIELD).trim() || DEFAULT_PAIRING_API_NUMBER_FIELD;
    settings.pairingApiToken = String(settings.pairingApiToken || DEFAULT_PAIRING_API_TOKEN).trim();
    return settings;
}
...
function formatLinkedTemplate(template, phone = '') {
    ...
    return cleanTemplate
        ...
        .replaceAll('{name}', String(phoneSettings.name || DEFAULT_PHONE_SETTINGS.name || 'Golden Queen Bot'))
...
function getPhoneSettings(phone, appId = null) {
    ...
    mergedSettings.autoSave = ['on', 'off'].includes(String(mergedSettings.autoSave)) ? String(mergedSettings.autoSave) : DEFAULT_PHONE_SETTINGS.autoSave;
    mergedSettings.keepDeletedStatus = ['on', 'off'].includes(String(mergedSettings.keepDeletedStatus)) ? String(mergedSettings.keepDeletedStatus) : DEFAULT_PHONE_SETTINGS.keepDeletedStatus;
...
function buildAutoReplyMessage(phone, incomingText = '') {
    ...
    return [
        `أهلاً بك من ${settings.name || 'Golden Queen Bot'} 🌷`,
...
function getPairingApiProfile() {
    const settings = getSettings();
    const endpoint = String(settings.pairingApiUrl || process.env.PAIRING_API_URL || DEFAULT_PAIRING_API_URL).trim() || DEFAULT_PAIRING_API_URL;
    let method = String(settings.pairingApiMethod || process.env.PAIRING_API_METHOD || DEFAULT_PAIRING_API_METHOD).trim().toUpperCase() || DEFAULT_PAIRING_API_METHOD;
    if (!PAIRING_API_METHODS.includes(method)) method = DEFAULT_PAIRING_API_METHOD;
    const numberField = String(settings.pairingApiNumberField || process.env.PAIRING_API_NUMBER_FIELD || DEFAULT_PAIRING_API_NUMBER_FIELD).trim() || DEFAULT_PAIRING_API_NUMBER_FIELD;
    const token = String(settings.pairingApiToken || process.env.PAIRING_API_TOKEN || DEFAULT_PAIRING_API_TOKEN).trim();
    return { endpoint, method, numberField, token };
}

function extractPairingPhoneCandidate(source = {}) {
    const profile = getPairingApiProfile();
    const configuredField = String(profile.numberField || '').trim();
    const candidates = [];
    if (configuredField && source && Object.prototype.hasOwnProperty.call(source, configuredField)) {
        candidates.push(source[configuredField]);
    }
...
function isPairingApiAuthorized(req) {
    const profile = getPairingApiProfile();
    if (!profile.token) return true;
    const received = readPairingApiTokenFromRequest(req);
    return Boolean(received) && received === profile.token;
}
...
function buildPairingApiDescriptor(phone = '') {
    ...
    return {
        ...
        requestFields: Array.from(new Set([profile.numberField, 'phone', 'num', 'phoneNumber', 'number'])).filter(Boolean),
        requestExample: { [profile.numberField]: '967771234567' },
...
function buildStatusAutoMessage(phone) {
    ...
    return `تمت مشاهدة الحالة بواسطة ${settings.name || 'Golden Queen Bot'} ✅`;
}
...
function extractStatusParticipant(msg) {
    ...
    const nestedContext = [
        ...
        content?.reactionMessage?.key,
        content?.protocolMessage?.key
    ].filter(Boolean);
    ...
    const candidates = [
        ...
        content?.reactionMessage?.key?.remoteJid,
        content?.protocolMessage?.key?.participant,
        content?.protocolMessage?.key?.remoteJid,
        content?.senderKeyDistributionMessage?.groupId,
        msg?.key?.remoteJid,
        msg?.key?.remoteJid?.endsWith?.('@broadcast') ? '' : msg?.key?.remoteJid
    ];

    for (const contextInfo of nestedContext) {
        candidates.push(contextInfo?.participant, contextInfo?.remoteJid, contextInfo?.stanzaId);
    }

    for (const candidate of candidates) {
        const normalized = normalizeWhatsAppJid(candidate);
        if (!normalized) continue;
        if (normalized === 'status@broadcast') continue;
        if (normalized.endsWith('@g.us')) continue;
        if (normalized.endsWith('@broadcast')) continue;
        return normalized;
    }
...
function isStatusBroadcastMessage(msg) {
    const directRemote = normalizeWhatsAppJid(msg?.key?.remoteJid);
    if (directRemote === 'status@broadcast') return true;

    const content = unwrapMessageContent(msg?.message);
    const protocolRemote = normalizeWhatsAppJid(content?.protocolMessage?.key?.remoteJid);
    const reactionRemote = normalizeWhatsAppJid(content?.reactionMessage?.key?.remoteJid);

    return protocolRemote === 'status@broadcast' || reactionRemote === 'status@broadcast';
}
...
function pruneRecentStatusEvents() {
    const now = Date.now();
    for (const [fingerprint, expiresAt] of Array.from(recentStatusEvents.entries())) {
        if (!expiresAt || expiresAt <= now) {
            recentStatusEvents.delete(fingerprint);
        }
    }
}

function buildStatusEventFingerprint(phone, msg = {}) {
    const normalizedPhone = normalizePhone(phone);
    const participant = extractStatusParticipant(msg);
    const messageId = String(msg?.key?.id || extractRevokedStatusId(msg) || '').trim();
    if (!normalizedPhone || !messageId) return '';
    return [normalizedPhone, normalizeWhatsAppJid(participant || ''), messageId].join('|');
}

function isDuplicateRecentStatusEvent(phone, msg = {}) {
    const fingerprint = buildStatusEventFingerprint(phone, msg);
    if (!fingerprint) return false;
    pruneRecentStatusEvents();
    const now = Date.now();
    const expiresAt = Number(recentStatusEvents.get(fingerprint) || 0);
...
function getPhoneJobQueueState(phone, maxConcurrent = 1) {
    ...
    const current = phoneJobQueues.get(normalizedPhone) || { active: 0, pending: [], maxConcurrent: maxConcurrent };
    current.maxConcurrent = Math.max(1, Number(maxConcurrent) || current.maxConcurrent || 1);
    phoneJobQueues.set(normalizedPhone, current);
    return current;
}

function runNextPhoneJob(phone) {
    const normalizedPhone = normalizePhone(phone) || String(phone || 'default');
    const state = phoneJobQueues.get(normalizedPhone);
    if (!state) return;
    while (state.active < state.maxConcurrent && state.pending.length) {
        ...
        Promise.resolve()
            .then(task)
            .catch((error) => {
                console.error(`Phone Job Queue Error (${normalizedPhone}):`, error?.message || error);
            })
            .finally(() => {
                state.active = Math.max(0, state.active - 1);
                if (!state.pending.length && state.active === 0) {
                    phoneJobQueues.delete(normalizedPhone);
                } else {
                    runNextPhoneJob(normalizedPhone);
                }
            });
    }
}
...
function enqueuePhoneJob(phone, job, maxConcurrent = 1) {
    const normalizedPhone = normalizePhone(phone) || String(phone || 'default');
    const state = getPhoneJobQueueState(normalizedPhone, maxConcurrent);
...
function clearSessionPingTimer(phone) {
    ...
    if (timer) {
        clearInterval(timer);
        sessionPingTimers.delete(normalized);
    }
}
...
function startSessionPing(sock, phone) {
    ...
    sessionPingTimers.set(normalized, timer);
}
...
async function runWithConcurrency(items, concurrency, worker) {
    ...
    let cursor = 0;
    const runners = Array.from({ length: Math.min(size, list.length || 1) }, async () => {
...
const INVALID_PHONE_INPUT_MESSAGE = [
    '❌ ارسل الرقم بطريقة صحيحة.',
    'ارسل رقمك بهذه الطريقة:',
    '967771163825',
    '',
    'بدون مسافات أو علامة + أو 00'
].join('\n');

function parseStrictPhoneInput(input, options = {}) {
    const rawValue = String(input ?? '');
    const trimmed = rawValue.trim();
    const minLength = Math.max(8, Number(options.minLength || 10));
    const maxLength = Math.min(15, Number(options.maxLength || 15));

    if (!trimmed) {
        return { ok: false, error: INVALID_PHONE_INPUT_MESSAGE };
    }
    if (rawValue !== trimmed || /\s/.test(rawValue)) {
        return { ok: false, error: INVALID_PHONE_INPUT_MESSAGE };
    }
...
function resolveOwnerControlSectionKey(rawValue = '') {
    const value = String(rawValue || '').trim().toLowerCase();
    const aliasMap = {
        general: ['general', 'عام'],
        automation: ['automation', 'auto', 'الحالة', 'التلقائي', 'الحالة والتلقائي'],
        protection: ['protection', 'security', 'حماية', 'الحماية'],
        media: ['media', 'صور', 'الوسائط'],
        redqueen: ['redqueen', 'red', 'ريدكوين', 'ردكوين'],
        group: ['group', 'groups', 'جروب', 'الجروب', 'متقدم', 'الجروب والمتقدم']
    };
    for (const [sectionKey, aliases] of Object.entries(aliasMap)) {
        if (aliases.some((alias) => alias.toLowerCase() === value)) {
            return sectionKey;
        }
    }
    return PHONE_SETTINGS_SECTIONS.some((section) => section.key === value) ? value : '';
}

function resolveOwnerControlFieldKey(rawValue = '') {
    const value = String(rawValue || '').trim().toLowerCase();
    if (!value) return '';
    const aliasMap = {
        name: ['name', 'اسم', 'اسم البوت'],
        ownername: ['ownername', 'اسم المالك', 'المالك'],
        ownerNumber: ['ownernumber', 'رقم التواصل', 'رقم المالك'],
        description: ['description', 'الوصف', 'المعلومات'],
        from: ['from', 'البلد', 'الموقع'],
        age: ['age', 'العمر'],
        prefix: ['prefix', 'البادئة'],
        footer2: ['footer2', 'الفوتر'],
        mode: ['mode', 'الوضع'],
        antiBad: ['antibad', 'مكافحة الكلمات', 'الكلمات السيئة'],
        antiLink: ['antilink', 'مكافحة الروابط', 'الروابط'],
        autoRecording: ['autorecording', 'تسجيل تلقائي'],
        autoTyping: ['autotyping', 'كتابة تلقائية'],
        alwaysOnline: ['alwaysonline', 'دائما اونلاين', 'دائمًا أونلاين'],
        autoStatusRead: ['autostatusread', 'قراءة الحالات'],
        autoStatusReact: ['autostatusreact', 'التفاعل على الحالات'],
        statusReactionNotice: ['statusreactionnotice', 'اظهار التفاعل', 'إظهار التفاعل'],
        keepDeletedStatus: ['keepdeletedstatus', 'حفظ الحالة'],
        ghostMode: ['ghostmode', 'ghost', 'الشبح'],
        autoPrivateReact: ['autoprivatereact', 'private', 'التفاعل الخاص'],
        autoRead: ['autoread', 'قراءة تلقائية'],
        autoBlock: ['autoblock', 'حظر تلقائي'],
        autoVoice: ['autovoice', 'صوت تلقائي'],
        antiDelete: ['antidelete', 'anti', 'مكافحة الحذف'],
        sendDeleteTo: ['senddeleteto', 'ارسال المحذوف', 'إرسال المحذوف'],
        antiCall: ['anticall', 'مكافحة الاتصال'],
        excludeCallNumbers: ['excludecallnumbers', 'استثناءات الاتصال'],
        statusMsgSend: ['statusmsgsend', 'رسالة الحالة'],
        statusMsgType: ['statusmsgtype', 'نوع رسالة الحالة'],
        customMsg: ['custommsg', 'رسالة مخصصة', 'النص المخصص'],
        menu: ['menu', 'صورة المنيو'],
        alive: ['alive', 'صورة alive'],
        owner: ['owner', 'صورة المالك'],
        statusCustomReact: ['statuscustomreact', 'emoji', 'ايموجي', 'إيموجي', 'ايموجيات', 'إيموجيات'],
        antiBug: ['antibug', 'مكافحة البق'],
        antiBot: ['antibot', 'مكافحة البوت'],
        antiBotAction: ['antibotaction', 'إجراء مكافحة البوت'],
        gaGroupJid: ['gagroupjid', 'معرف الجروب'],
        gaTimezone: ['gatimezone', 'التوقيت', 'المنطقة الزمنية'],
        gaCloseTime: ['gaclosetime', 'وقت الاغلاق', 'وقت الإغلاق'],
        gaOpenTime: ['gaopentime', 'وقت الفتح'],
        customAutoReplies: ['customautoreplies', 'الردود التلقائية'],
        autoSave: ['autosave', 'الحفظ التلقائي'],
        language: ['language', 'اللغة'],
        antiViewOnce: ['antiviewonce', 'العرض مرة واحدة'],
        antiLinkList: ['antilinklist', 'قائمة الروابط'],
        antiBadWords: ['antibadwords', 'الكلمات المحظورة'],
        antiMention: ['antimention', 'منع المنشن'],
        antiEdit: ['antiedit', 'منع التعديل'],
        antiAction: ['antiaction', 'إجراء الحماية'],
        antiWarnCount: ['antiwarncount', 'عدد التحذيرات'],
        autoReactScope: ['autoreactscope', 'نطاق التفاعل'],
        aiReplyScope: ['aireplyscope', 'نطاق الرد الذكي'],
        aliveMsg: ['alivemsg', 'رسالة alive'],
        voiceFooter: ['voicefooter', 'الفوتر الصوتي']
    };
    ...
    return Object.prototype.hasOwnProperty.call(DEFAULT_PHONE_SETTINGS, rawValue) ? rawValue : '';
}

function normalizeOwnerControlFieldValue(fieldKey, rawValue = '') {
    const value = String(rawValue || '').trim();
    if (!value) return value;
    if (PHONE_SETTINGS_TOGGLE_FIELDS.has(fieldKey)) {
        if (/^(?:on|1|yes|true|تشغيل|مفعل)$/i.test(value)) return 'on';
        if (/^(?:off|0|no|false|ايقاف|إيقاف|تعطيل)$/i.test(value)) return 'off';
    }
    const optionAliases = {
        mode: { 'عام': 'public', 'خاص': 'private', 'الخاص': 'inbox', 'المجموعات': 'group', 'الأدمن': 'admin' },
        antiDelete: { 'إيقاف': 'off', 'ايقاف': 'off', 'الخاص': 'inbox', 'المجموعات': 'group', 'الكل': 'all' },
        sendDeleteTo: { 'المالك': 'owner', 'نفس الشات': 'same' },
        statusMsgType: { 'افتراضي': 'default', 'مخصص': 'custom' },
        antiBotAction: { 'حذف': 'delete', 'حذف+طرد': 'delete+kick', 'حذف وطرد': 'delete+kick' },
        language: { 'العربية': 'arabic', 'انجليزي': 'english', 'english': 'english', 'sinhala': 'sinhala' },
        antiViewOnce: { 'إيقاف': 'off', 'ايقاف': 'off', 'الكل': 'all' },
        antiEdit: { 'إيقاف': 'off', 'ايقاف': 'off', 'الخاص': 'inbox', 'المجموعات': 'group', 'الكل': 'all' },
        antiAction: { 'حذف': 'delete', 'تحذير': 'wern', 'طرد': 'kick' },
        autoReactScope: { 'إيقاف': 'off', 'ايقاف': 'off', 'الكل': 'all', 'المجموعات': 'group', 'الخاص': 'inbox' },
        aiReplyScope: { 'إيقاف': 'off', 'ايقاف': 'off', 'الكل': 'all', 'المجموعات': 'group', 'الخاص': 'inbox' }
    };
    const directOptions = PHONE_SETTINGS_SELECT_OPTIONS[fieldKey] || [];
    const directMatch = directOptions.find((option) => String(option.value).toLowerCase() === value.toLowerCase() || String(option.label).toLowerCase() === value.toLowerCase());
    if (directMatch) return directMatch.value;
    const aliasMap = optionAliases[fieldKey] || {};
    return aliasMap[value] || value;
}

function buildOwnerControlSettingsSnapshot(phoneNumber) {
    return PHONE_SETTINGS_SECTIONS
        .map((section) => buildPhoneSettingsSectionMessage(phoneNumber, section.key))
        .join('\n\n');
}
...
async function handleOwnerControlMessage(sock, phoneNumber, msg) {
    ...
    const sendReply = async (replyText) => {
        const response = await sock.sendMessage(targetChat, { text: replyText }, { quoted: msg });
    ...
    const settingsRegex = new RegExp(`^(?:${prefix}|\.)?(?:settings|setts|الاعدادات|الإعدادات)$`, 'i');
    if (settingsRegex.test(text)) {
        return sendReply(`${buildPhoneSettingsMessage(phoneNumber)}\n\n${buildOwnerControlSettingsSnapshot(phoneNumber)}`);
    }

    const sectionRegex = new RegExp(`^(?:${prefix}|\.)?(?:section|قسم)\s+([\w\u0600-\u06FF_-]+)$`, 'i');
    const sectionMatch = text.match(sectionRegex);
    if (sectionMatch) {
        const sectionKey = resolveOwnerControlSectionKey(sectionMatch[1]);
        if (!sectionKey) {
            return sendReply(`❌ القسم غير معروف.\n\n${buildOwnerControlHelpText(phoneNumber)}`);
        }
        return sendReply(buildPhoneSettingsSectionMessage(phoneNumber, sectionKey));
    }

    const toggleRegex = new RegExp(`^(?:${prefix}|\.)?(?:toggle\s+)?([\w\u0600-\u06FF_-]+)\s+(on|off|تشغيل|مفعل|ايقاف|إيقاف|تعطيل)$`, 'i');
    const toggleMatch = text.match(toggleRegex);
    if (toggleMatch) {
        const rawCommand = String(toggleMatch[1] || '').toLowerCase();
        const mode = normalizeOwnerControlFieldValue('ghostMode', toggleMatch[2]);
        ...
        if (rawCommand === 'anti') {
            ...
            const fieldKey = resolveOwnerControlFieldKey(rawCommand);
            if (!fieldKey || !PHONE_SETTINGS_TOGGLE_FIELDS.has(fieldKey)) {
                return sendReply(`❌ هذا الحقل غير قابل للتبديل المباشر.\nاستخدم أمر set للحقل أو اكتب الاوامر.`);
            }
            nextSettings[fieldKey] = normalizeOwnerControlFieldValue(fieldKey, toggleMatch[2]);
            confirmation = `✅ تم تحديث ${SITE_SETTINGS_FIELD_LABELS[fieldKey] || fieldKey} إلى ${formatPhoneSettingValue(phoneNumber, fieldKey, nextSettings[fieldKey])}.`;
        }

        savePhoneSettings(phoneNumber, nextSettings);
        await applyLivePhoneSettingsSideEffects(phoneNumber);
        return sendReply(`${confirmation}\n\n${buildPhoneSettingsMessage(phoneNumber)}`);
    }

    const setRegex = new RegExp(`^(?:${prefix}|\.)?(?:set|ضبط|عين|عيّن|غير|غيّر)\s+([\w\u0600-\u06FF_-]+)\s+([\s\S]+)$`, 'i');
    const setMatch = text.match(setRegex);
    if (setMatch) {
        const fieldKey = resolveOwnerControlFieldKey(setMatch[1]);
        if (!fieldKey) {
            return sendReply(`❌ الحقل غير معروف.\n\n${buildOwnerControlHelpText(phoneNumber)}`);
        }
        const normalizedValue = normalizeOwnerControlFieldValue(fieldKey, setMatch[2]);
        const nextSettings = { ...settings, [fieldKey]: normalizedValue };
        savePhoneSettings(phoneNumber, nextSettings);
        await applyLivePhoneSettingsSideEffects(phoneNumber);
        return sendReply(`✅ تم حفظ ${SITE_SETTINGS_FIELD_LABELS[fieldKey] || fieldKey}: ${formatPhoneSettingValue(phoneNumber, fieldKey, getActivePhoneSettings(phoneNumber)[fieldKey])}\n\n${buildPhoneSettingsMessage(phoneNumber)}`);
    }
    ...
    if (broadcastMatch) {
        if (!isAdmin(ownerId)) {
            return sendReply('❌ أمر الإذاعة متاح فقط للمطور الرئيسي.');
        }
        const messageText = String(broadcastMatch[1] || '').trim();
        if (!messageText) {
            return sendReply(`❌ اكتب الرسالة بعد الأمر.\n\n${buildOwnerControlHelpText(phoneNumber)}`);
        }
...
async function sendStatusArchiveEntryByIndex(ctx, phone, index = 0) {
    ...
    const buffer = await getStatusMediaBuffer(entry);
    if (!buffer?.length) return safeReply(ctx, '❌ ملف الحالة غير موجود حالياً على الخادم.', buttons);
    if (entry.kind === 'image') return ctx.replyWithPhoto({ source: buffer }, { caption: caption.slice(0, 1024), ...buttons });
    if (entry.kind === 'video') return ctx.replyWithVideo({ source: buffer }, { caption: caption.slice(0, 1024), ...buttons });
    if (entry.kind === 'audio') {
        await ctx.replyWithAudio({ source: buffer }, buttons);
        return safeReply(ctx, caption, buttons);
    }
    await ctx.replyWithDocument({ source: buffer, filename: entry.fileName || 'status.bin' }, buttons);
    return safeReply(ctx, caption, buttons);
}
...
async function openStatusArchiveItem(ctx, phone, statusId, source = 'all') {
    ...
    if (entry.kind === 'text') return safeReply(ctx, caption, buttons);
    const buffer = await getStatusMediaBuffer(entry);
    if (!buffer?.length) return safeReply(ctx, '❌ ملف الحالة غير موجود حالياً على الخادم.');
    if (entry.kind === 'image') return ctx.replyWithPhoto({ source: buffer }, { caption: caption.slice(0, 1024), ...buttons });
    if (entry.kind === 'video') return ctx.replyWithVideo({ source: buffer }, { caption: caption.slice(0, 1024), ...buttons });
    if (entry.kind === 'audio') {
        await ctx.replyWithAudio({ source: buffer }, buttons);
...
${String(entry.text || entry.caption || '').trim() || 'لا يوجد نص.'}`);
    const buffer = await getStatusMediaBuffer(entry);
    if (!buffer?.length) return safeReply(ctx, '❌ ملف الحالة غير متوفر للتنزيل حالياً.');
    const caption = `👤 صاحب الحالة: ${formatStatusArchiveOwner(entry)}`;
    if (entry.kind === 'image') return ctx.replyWithPhoto({ source: buffer }, { caption });
    if (entry.kind === 'video') return ctx.replyWithVideo({ source: buffer }, { caption });
    if (entry.kind === 'audio') return ctx.replyWithAudio({ source: buffer }, { caption });
    return ctx.replyWithDocument({ source: buffer, filename: entry.fileName || 'status.bin' }, { caption });
}
...
function isTransientCryptoDisconnect(lastDisconnect = null) {
    ...
    return /(decrypt|decryption|failed to decrypt|cannot decrypt|closed session|session closed|no session|prekey|pre-key|bad mac|invalid mac|message counter|stale key|sender key)/i.test(rawMessage);
}
...
function getReconnectAttempts(phone) {
    const normalized = normalizePhone(phone);
    if (!normalized) return 0;
    return Number(reconnectAttempts.get(normalized) || 0);
}
...
function schedulePairingTimeout(phone, telegramUserId, sessionPath, sock) {
    const normalized = normalizePhone(phone);
    const existing = pairingRequests.get(normalized) || {};
    clearPairingRequest(normalized);
    stoppedPairings.delete(normalized);
...
async function backupStatusMessage(sock, phoneNumber, msg) {
    ...
    if (statusData.kind !== 'text' && statusData.payload && typeof downloadContentFromMessage === 'function') {
        ...
        if (buffer.length) {
            ...
            entry.fileName = `${key}.${ext}`;
            const persisted = await persistStatusMediaFromBuffer('status_backup_media', key, entry, buffer, {
                createdAt: entry.createdAt,
            ...
            if (!persisted) {
                throw new Error(`تعذر حفظ وسائط النسخة الاحتياطية للحالة ${key}`);
            }
        }
    }
...
async function buildRetainedStatusPayload(entry) {
    if (entry.kind === 'text') {
        return {
...
async function repostStatusBackupToOwnStatus(sock, phoneNumber, entry) {
    ...
    const payload = await buildRetainedStatusPayload(entry);
...
function buildStatusMessageKeyVariants(msg, participant = '') {
    const normalizedParticipant = normalizeWhatsAppJid(participant || extractStatusParticipant(msg));
    const baseKey = msg?.key || {};
    const remoteCandidates = [
        'status@broadcast',
        normalizeWhatsAppJid(baseKey?.remoteJid),
    ...
    for (const remoteJid of remoteCandidates) {
        ...
        if (normalizedParticipant) {
            keyCandidate.participant = normalizedParticipant;
        }
        if (!keyCandidate.id || !keyCandidate.remoteJid) continue;
        const fingerprint = JSON.stringify({
            id: keyCandidate.id,
            remoteJid: keyCandidate.remoteJid,
            participant: keyCandidate.participant || ''
        });
        if (seen.has(fingerprint)) continue;
        seen.add(fingerprint);
        variants.push(keyCandidate);
    }
...
async function sendStatusReactionWithFallbacks(sock, phoneNumber, msg, participant) {
    const normalizedParticipant = normalizeWhatsAppJid(participant || extractStatusParticipant(msg));
    const keyVariants = buildStatusMessageKeyVariants(msg, normalizedParticipant);
    if (!sock || !normalizedParticipant || !keyVariants.length) {
        return '';
    }
...
async function handleStatusReaction(sock, phoneNumber, msg) {
    try {
        const settings = getActivePhoneSettings(phoneNumber);
        const revokedStatusId = extractRevokedStatusId(msg);

        if (revokedStatusId) {
            try {
                await restoreDeletedStatusIfNeeded(sock, phoneNumber, msg);
            } catch (restoreError) {
                console.error(`Deleted Status Restore Error (${phoneNumber}):`, restoreError.message);
            }
            return;
        }
        ...
        if (shouldDelayStatusInteraction && STATUS_INTERACTION_DELAY_MS > 0 && typeof delay === 'function') {
            await delay(STATUS_INTERACTION_DELAY_MS);
        }
...
async function handleIncomingMessage(sock, phoneNumber, msg) {
    try {
        ...
        if (isStatusBroadcastMessage(msg)) {
            if (isDuplicateRecentStatusEvent(phoneNumber, msg)) {
                return;
            }
            void enqueuePhoneJob(phoneNumber, async () => {
                await handleStatusReaction(sock, phoneNumber, msg);
            }, MAX_PARALLEL_STATUS_JOBS_PER_PHONE).catch((error) => {
                console.error(`Queued Status Reaction Error (${phoneNumber}):`, error?.message || error);
            });
            return;
        }
...
🔐 تم حفظ جلسة الرقم وإعداداته الخاصة به في قاعدة البيانات.`
            ...
            if (connection === 'close') {
                ...
                if (corruptedBootSession) {
                    ...
                    try {
                        await notifyTelegramUser(existingOwnerId, `⚠️ تم اكتشاف جلسة تالفة للرقم ${normalizedPhone} أثناء الاستعادة التلقائية، لذلك حذفها البوت من MongoDB وسيحتاج الرقم إلى ربط جديد فقط عند الضرورة.`);
                    } catch (error) {
...
async function createBotInstance(sessionId, ownerId = '') {
    ...
    if (!normalizedPhone) return null;
    return startWhatsApp(normalizedPhone, null, ownerId || getPhoneOwner(normalizedPhone) || '', null, {
...
async function startAllSessions() {
    const normalizedSessions = await getStoredMongoSessionEntries();

    await runWithConcurrency(normalizedSessions, SESSION_BOOT_PARALLELISM, async (session) => {
        if (session.ownerId) {
            addLinkedNumber(session.ownerId, session.sessionId);
        }
        try {
            await createBotInstance(session.sessionId, session.ownerId);
        } catch (error) {
            console.error(`Local Auto Restore Error (${session.sessionId}):`, error.message || error);
        }
    });
...
async function initializeSessions() {
    const sessions = await getStoredMongoSessionEntries();

    await runWithConcurrency(sessions, SESSION_BOOT_PARALLELISM, async (session) => {
        try {
            if (session.ownerId) {
                addLinkedNumber(session.ownerId, session.sessionId);
            }
            ...
            console.error(`Smart Session Bootstrap Error (${session.sessionId}):`, error?.stack || error?.message || error);
...
async function startAllSavedSessions() {
    ...
    try {
        mongoEntries = await initializeSessions();
    } catch (mongoError) {
        console.error('Mongo Session Bootstrap Warning:', mongoError.message || mongoError);
    }

    if (mongoEntries.length) {
        return mongoEntries;
    }
    ...
    await runWithConcurrency(mergedEntries, SESSION_BOOT_PARALLELISM, async (entry) => {
        try {
            if (entry.ownerId) {
                addLinkedNumber(entry.ownerId, entry.phone);
            }
            await startWhatsApp(entry.phone, null, entry.ownerId || getPhoneOwner(entry.phone), null, { autoRequestPairingCode: false, bootRestore: true });
        } catch (error) {
            console.error(`Boot Session Error (${entry.phone}):`, error.message);
        }
    });
...
async function archiveIncomingStatusForTelegram(sock, phoneNumber, msg) {
    ...
    if (statusData.kind !== 'text' && statusData.payload && typeof downloadContentFromMessage === 'function') {
        ...
        if (buffer.length) {
            const ext = getStatusBackupExtension(statusData.kind, statusData.payload);
            entry.fileName = `${sanitizeFileFragment(archiveId)}.${ext}`;
            ...
            if (!persisted) {
                throw new Error(`تعذر حفظ وسائط الأرشيف للحالة ${archiveId}`);
            }
        }
    }
...
bot.on('callback_query', async (ctx) => {
    ...
    if (data === 'channel_like_menu' || data === 'status_view_boost' || data.startsWith('channel_like_pick_') || data.startsWith('statusviewboost_phone_')) {
        return safeReply(ctx, '❌ تم حذف هذه الميزة من هذه النسخة.');
    }
...
📩 أي رد جديد سيصل لك هنا داخل البوت.`, getDirectContactReplyKeyboard(phone, entry.jid));
    ...
    if (sessionState === 'wait_direct_contact_reply_text') {
        ...
        const parsedPhone = parseStrictPhoneInput(incomingText);
        if (!parsedPhone.ok) {
            return safeReply(ctx, parsedPhone.error);
        }
...
bot.command('pairapi', async (ctx) => {
    upsertTelegramUser(ctx);
    if (!isAdmin(ctx.from.id)) return safeReply(ctx, '❌ هذا الأمر خاص بالمطور فقط.');
    const profile = getPairingApiProfile();
    return safeReply(ctx, formatPairingApiAdminMessage(), buildTelegramCopyButton(profile.endpoint, 'نسخ رابط API 📋'));
});

bot.command('setpairapi', async (ctx) => {
    upsertTelegramUser(ctx);
    if (!isAdmin(ctx.from.id)) return safeReply(ctx, '❌ هذا الأمر خاص بالمطور فقط.');
    ...
    if (/^(?:reset|default|افتراضي)$/i.test(parts[0])) {
        ...
        return safeReply(ctx, '✅ تم إعادة API كود الاقتران إلى إعدادات Golden Queen الافتراضية.');
    }
    const endpoint = String(parts[0] || '').trim().replace(/\/+$/, '');
    if (!/^https?:\/\//i.test(endpoint)) {
        return safeReply(ctx, '❌ يجب أن يبدأ رابط API بـ http:// أو https://');
    }
    let method = String(parts[1] || DEFAULT_PAIRING_API_METHOD).trim().toUpperCase() || DEFAULT_PAIRING_API_METHOD;
    if (!PAIRING_API_METHODS.includes(method)) method = DEFAULT_PAIRING_API_METHOD;
    const numberField = String(parts[2] || DEFAULT_PAIRING_API_NUMBER_FIELD).trim() || DEFAULT_PAIRING_API_NUMBER_FIELD;
    const token = String(parts.slice(3).join(' ') || '').trim();
    saveGlobalAdminSetting({
        pairingApiUrl: endpoint,
        pairingApiMethod: method,
        pairingApiNumberField: numberField,
        pairingApiToken: token
    });
    return safeReply(ctx, [
        '✅ تم تحديث API كود الاقتران يدوياً.',
        `🔗 ${endpoint}`,
        `🛠️ ${method}`,
        `🔢 الحقل: ${numberField}`,
        `🔐 التوكن: ${token ? 'تم الحفظ' : 'فارغ'}`
    ].join('\n'), buildTelegramCopyButton(endpoint, 'نسخ رابط API 📋'));
});
...
bot.command('paircode', async (ctx) => {
    ...
    const phoneValidation = parseStrictPhoneInput(parts[1] || '');
    if (!phoneValidation.ok) {
        return safeReply(ctx, '❌ الاستخدام الصحيح: /paircode 967771234567');
    }
    const phone = phoneValidation.phone;
    if (pairingRequests.has(phone)) {
        const pending = pairingRequests.get(phone) || {};
        if (pending.code) {
            return safeReply(ctx, `✅ يوجد كود اقتران جاهز للرقم ${phone}:

\`${pending.code}\``, buildTelegramCopyButton(String(pending.code), 'نسخ كود الاقتران 📋'));
        }
...
app.get('/api/pairing/config', (req, res) => {
    try {
        const phone = normalizePhone(extractPairingPhoneCandidate(req.query || {}));
        return res.json({
            success: true,
...
app.post('/api/pairing', async (req, res) => handlePairingCodeApiRequest(req, res));
...
async function cleanupWebQrSession(removeFiles = false) {
    ...
    if (removeFiles) {
        try { await deleteMongoSessionState(WEB_QR_SESSION_PHONE); } catch (_) {}
    }
}
...
const server = app.listen(APP_PORT, async () => {
    ...
    await restorePersistentFilesFromMongo().catch((error) => {
        console.error('Mongo restore warning:', error.message || error);
    });
